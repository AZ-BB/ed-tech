-- Add sessions_and_calls KPI to advisor_dashboard (all-time sessions/calls total).
-- Includes completed sessions/calls; excludes cancelled advisor sessions.
...
BEGIN
  ...
  booked_meetings AS (
    ...
    WHERE ads.advisor_id = v_advisor_id
      ...
      AND coalesce(ads.status, '') <> 'cancelled'
    UNION ALL
