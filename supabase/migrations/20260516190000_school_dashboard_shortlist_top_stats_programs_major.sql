-- Top "programs" for school_dashboard_shortlist_top_stats must reflect major/program lines
-- from the My Applications shortlist (`student_shortlist_universities.major_program`), not
-- catalog university names (`universities.name`). See 20260516130000 for the regression.
...
COMMENT ON FUNCTION public.school_dashboard_shortlist_top_stats(integer) IS
  'School portal: top destination countries from discovery university shortlists (student_activities); top programs from workspace major_program on student_shortlist_universities; shortlist_row_count is distinct (student, uni) discovery pairs. Uses SECURITY DEFINER; scoped by current_school_admin_school_id().';
