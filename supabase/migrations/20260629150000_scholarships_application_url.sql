-- Dedicated application URL for scholarships (apply button + Excel import/export).
-- Existing URLs live in discovery_payload.applicationUrl; backfill the column from there.

ALTER TABLE public.scholarships
  ADD COLUMN IF NOT EXISTS application_url text;

UPDATE public.scholarships
SET application_url = nullif(trim(discovery_payload->>'applicationUrl'), '')
WHERE application_url IS NULL
  AND nullif(trim(coalesce(discovery_payload->>'applicationUrl', '')), '') IS NOT NULL;

-- Include application_url in discovery RPC rows (fallback when discovery_payload is absent).
  ...
  WITH base AS (
    SELECT
      ...
      CASE
        ...
        THEN (
          SELECT coalesce(jsonb_agg(sd.country_code ORDER BY sd.country_code), '[]'::jsonb)
          FROM public.scholarship_destinations sd
          WHERE sd.scholarship_id = s.id
        )
        ELSE '["Global"]'::jsonb
