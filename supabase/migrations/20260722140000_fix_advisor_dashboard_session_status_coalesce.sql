-- Fix advisor_dashboard: coalesce(ads.status, '') cast '' to advisor_session_status enum when status is NULL.
-- Use IS DISTINCT FROM so NULL sessions are still counted (same behavior as before).
...
DECLARE
  ...
  v_week_end date := (date_trunc('week', v_today) + interval '7 days')::date;
  v_month_start date := date_trunc('month', v_today)::date;
  v_month_end date := (date_trunc('month', v_today) + interval '1 month')::date;
  v_empty jsonb := jsonb_build_object(
    'kpis', jsonb_build_object(
      'sessions_and_calls', jsonb_build_object('total', 0, 'this_week', 0),
...
BEGIN
  ...
  lead_apps AS (
    ...
    WHERE ae.status::text = 'lead'
  ),
  active_package_apps AS (
    SELECT ae.*
    FROM app_enriched ae
    WHERE ae.status::text = 'active_package'
  ),
  booked_meetings AS (
    -- All-time advisor sessions (include completed; exclude cancelled)
    SELECT ads.booked_at AS meeting_at
    FROM public.advisor_sessions ads
    WHERE ads.advisor_id = v_advisor_id
      AND ads.booked_at IS NOT NULL
      AND ads.status IS DISTINCT FROM 'cancelled'
    UNION ALL
    -- All applications that were ever Calendly-scheduled
    SELECT a.scheduled_at AS meeting_at
    FROM apps a
    WHERE a.scheduled_at IS NOT NULL
    UNION ALL
    -- All post-admission cases that were ever Calendly-scheduled
    SELECT pac.scheduled_at AS meeting_at
    FROM public.post_admission_cases pac
    WHERE pac.assigned_to = v_advisor_id
      AND pac.scheduled_at IS NOT NULL
    UNION ALL
    -- Logged application calls (all statuses)
    SELECT ac.call_date::timestamptz AS meeting_at
    FROM public.application_calls ac
    INNER JOIN apps a ON a.id = ac.application_id
    UNION ALL
    -- Logged post-admission calls (all statuses)
    SELECT (pacall.call_date::timestamp AT TIME ZONE 'UTC') AS meeting_at
    FROM public.post_admission_calls pacall
    INNER JOIN public.post_admission_cases pac
      ON pac.id = pacall.post_admission_case_id
    WHERE pac.assigned_to = v_advisor_id
  ),
  completed_calls AS (
    SELECT
  ...
  metrics AS (
    SELECT
      (SELECT count(*)::int FROM booked_meetings) AS sessions_and_calls_total,
      (
        SELECT count(*)::int
        FROM booked_meetings bm
        WHERE bm.meeting_at::date >= v_week_start
          AND bm.meeting_at::date < v_week_end
      ) AS sessions_and_calls_this_week,
  ...
  deadline_rows AS (
    ...
    SELECT
      ...
      (
        SELECT a.id
        FROM apps a
        WHERE a.student_id = ssu.student_id
        ORDER BY
          CASE WHEN a.status::text = 'active_package' THEN 0 ELSE 1 END,
          a.id DESC
        LIMIT 1
      ) AS application_id,
      ssu.university_name,
  ...
  SELECT jsonb_build_object(
    'kpis', jsonb_build_object(
      'sessions_and_calls', jsonb_build_object(
        'total', m.sessions_and_calls_total,
        'this_week', m.sessions_and_calls_this_week
      ),
