-- Simplify application_status to lead / not_suitable / payment_requested / active_package.
...
ALTER TABLE public.applications
  ...
  USING (
    CASE status::text
      WHEN 'new' THEN 'lead'::public.application_status_new
      WHEN 'scheduled' THEN 'lead'::public.application_status_new
      WHEN 'payment_in_progress' THEN 'payment_requested'::public.application_status_new
      WHEN 'payment_completed' THEN 'active_package'::public.application_status_new
      WHEN 'in_progress' THEN 'active_package'::public.application_status_new
      WHEN 'submitted' THEN 'active_package'::public.application_status_new
      WHEN 'blocked' THEN 'not_suitable'::public.application_status_new
      ELSE 'lead'::public.application_status_new
    END
  );
...
COMMENT ON COLUMN public.applications.payment_in_progress_at IS
  'When the application status was set to payment_requested.';

COMMENT ON COLUMN public.applications.payment_completed_at IS
  'When the application status was set to active_package.';

COMMENT ON COLUMN public.applications.blocked_at IS
  'When the application status was set to not_suitable.';

-- Advisor dashboard: use simplified application statuses.
...
-- Advisor students table: derive management_status from application status.
...
BEGIN
  ...
  status_by_student AS (
    SELECT
      a.student_id,
      CASE
        WHEN bool_or(a.status::text = 'active_package') THEN 'active_package'
        WHEN bool_or(a.status::text = 'payment_requested') THEN 'payment_requested'
        WHEN bool_or(a.status::text = 'not_suitable') THEN 'not_suitable'
        ELSE 'lead'
