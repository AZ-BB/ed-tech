-- Advisor in-app notifications for assigned-student application and session activity.
...
CREATE UNIQUE INDEX advisor_notifications_source_unique
  ON public.advisor_notifications (advisor_id, source_table, source_id, event_type);

CREATE INDEX advisor_notifications_advisor_created_idx
  ON public.advisor_notifications (advisor_id, created_at DESC);

CREATE TABLE public.advisor_notification_reads (
...
CREATE INDEX advisor_notification_reads_advisor_idx
...
COMMENT ON FUNCTION public.create_advisor_notification(uuid, uuid, text, text, text, text, bigint) IS
  'Creates an advisor-scoped notification for assigned-student activity; deduped by advisor + source.';

-- Notify all advisors with an assigned application for this student.
