"use server";

import {
  createSupabaseSecretClient,
  createSupabaseServerClient,
} from "@/utils/supabase-server";
import {
  recordStudentPlatformCompletionOnce,
  STUDENT_PLATFORM_COMPLETION_FLAGS,
} from "@/lib/student-platform-completion";

export type InternshipActivityActionResult =
  | { ok: true }
  | { ok: false; error: string };

async function requireStudent(): Promise<
  | {
      ok: true;
      supabase: Awaited<ReturnType<typeof createSupabaseServerClient>>;
      studentId: string;
    }
  | { ok: false; error: string }
> {
  const supabase = await createSupabaseServerClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user?.id) {
    return { ok: false, error: "You must be signed in." };
  }
  return { ok: true, supabase, studentId: user.id };
}

async function insertActivityLog(
  studentId: string,
  internshipUuid: string,
  action: string,
  message: string,
): Promise<{ ok: false; error: string } | null> {
  const supabase = await createSupabaseSecretClient();
  const { error } = await supabase.from("acitivity_logs").insert({
    action,
    message,
    entitiy_type: "internship",
    entity_id: internshipUuid,
    created_by_type: "student",
    student_id: studentId,
    admin_id: null,
    school_admin_id: null,
  });
  if (error) {
    const msg =
      typeof error.message === "string" && error.message
        ? error.message
        : "Failed to write activity log.";
    console.error("[insertActivityLog]", msg);
    return { ok: false, error: msg };
  }
  return null;
}

/** Resolves `internships.id` (UUID) from a discovery UI id (slug or row UUID). */
async function resolveInternshipUuidForDiscoveryId(
  discoveryId: string,
): Promise<string | null> {
  const supabase = await createSupabaseServerClient();
  const uuidRe =
    /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
  if (uuidRe.test(discoveryId)) {
    const { data, error } = await supabase
      .from("internships")
      .select("id")
      .eq("id", discoveryId)
      .maybeSingle();
    if (!error && data?.id) return data.id;
  }
  const { data: bySlug } = await supabase
    .from("internships")
    .select("id")
    .eq("slug", discoveryId)
    .maybeSingle();
  return bySlug?.id ?? null;
}

function markViewedInternships(
  supabase: Awaited<ReturnType<typeof createSupabaseServerClient>>,
  studentId: string,
) {
  recordStudentPlatformCompletionOnce(
    supabase,
    studentId,
    STUDENT_PLATFORM_COMPLETION_FLAGS.viewed_internships,
  ).catch(() => {});
}

/** Records `student_activities` (type `save`) + `acitivity_logs`. */
export async function saveInternship(
  discoveryId: string,
): Promise<InternshipActivityActionResult> {
  const auth = await requireStudent();
  if (!auth.ok) return auth;

  const internshipId = await resolveInternshipUuidForDiscoveryId(
    discoveryId.trim(),
  );
  if (!internshipId) {
    return { ok: false, error: "Internship not found." };
  }

  const { supabase, studentId } = auth;

  const { data: existing } = await supabase
    .from("student_activities")
    .select("id")
    .eq("student_id", studentId)
    .eq("entity_type", "internship")
    .eq("type", "save")
    .eq("internship_id", internshipId)
    .maybeSingle();

  if (existing) {
    markViewedInternships(supabase, studentId);
    return { ok: true };
  }

  const { error: actError } = await supabase.from("student_activities").insert({
    student_id: studentId,
    entity_type: "internship",
    type: "save",
    internship_id: internshipId,
    scholarship_id: null,
    uni_id: null,
    advisor_id: null,
    ambassador_id: null,
  });

  if (actError) {
    console.error("[saveInternship]", actError.message);
    return { ok: false, error: actError.message };
  }

  const logErr = await insertActivityLog(
    studentId,
    internshipId,
    "internship_saved",
    "Student saved an internship to their list.",
  );
  if (logErr) return logErr;

  markViewedInternships(supabase, studentId);

  return { ok: true };
}

/** Removes a `save` activity for this internship. */
export async function unsaveInternship(
  discoveryId: string,
): Promise<InternshipActivityActionResult> {
  const auth = await requireStudent();
  if (!auth.ok) return auth;

  const internshipId = await resolveInternshipUuidForDiscoveryId(
    discoveryId.trim(),
  );
  if (!internshipId) {
    return { ok: false, error: "Internship not found." };
  }

  const { supabase, studentId } = auth;

  const { error } = await supabase
    .from("student_activities")
    .delete()
    .eq("student_id", studentId)
    .eq("entity_type", "internship")
    .eq("type", "save")
    .eq("internship_id", internshipId);

  if (error) {
    console.error("[unsaveInternship]", error.message);
    return { ok: false, error: error.message };
  }

  const logErr = await insertActivityLog(
    studentId,
    internshipId,
    "internship_unsaved",
    "Student removed an internship from their saved list.",
  );
  if (logErr) return logErr;

  return { ok: true };
}
