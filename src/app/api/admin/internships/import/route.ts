...
import { csvToRecords, importInternshipsFromCsvRecords } from "@/lib/internship-csv-import";

export const maxDuration = 600;

const LOG = "[admin-internships-import]";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const stream = createImportSseStream(async (send) => {
      ...
      const summary = await importInternshipsFromCsvRecords(auth.service, records, {
        onProgress,
      });
