...
export default async function PostAdmissionSupportPaymentCancelPage({
  searchParams,
}: PageProps) {
  const sp = await searchParams;
  const raw =
    typeof sp.case_id === "string"
      ? sp.case_id
      : Array.isArray(sp.case_id)
        ? sp.case_id[0]
        : undefined;
  const caseId = raw ? Number.parseInt(raw, 10) : NaN;
  const refLabel =
        ...
        <div className="mx-auto flex w-full max-w-xl flex-1 flex-col justify-center py-6 sm:py-8">
          <div className="rounded-[14px] border border-[var(--border-light)] bg-white px-6 py-8 text-center">
            ...
            <p className="mt-2 text-sm text-[var(--text-mid)]">
              ...
              in your email when you are ready to complete the payment.
            </p>
            <Link
              href="/contact"
