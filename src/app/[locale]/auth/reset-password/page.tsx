...
function ResetPasswordContent() {
  ...
  const establishSession = useCallback(async () => {
    ...
    if (code) {
      ...
      if (error) {
        setLoadError(a.resetLinkInvalid);
        setReady(true);
        return;
      }
      router.replace(localizePath("/auth/reset-password", locale));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (password.length < MIN_PASSWORD) {
      setSubmitError(a.passwordMinLength.replace("{min}", String(MIN_PASSWORD)));
      return;
    }
    if (password !== confirmPassword) {
      setSubmitError(a.passwordsDoNotMatch);
      return;
    }
    ...
    >
      <div className="relative z-10 flex w-full max-w-[min(100%,28rem)] flex-col items-center px-4 py-10 sm:px-5 sm:py-12">
        ...
        <div className="w-full rounded-[20px] border border-[var(--border-light)] bg-white p-8 shadow-[0_4px_24px_rgba(0,0,0,0.06)] sm:p-10">
          {!ready ? (
            <p className="text-center text-sm text-[#6a6a6a]">{a.verifyingLink}</p>
          ) : done ? (
            <div className="text-center">
              <h1 className="serif text-[1.6rem] leading-tight text-[var(--text)]">
                {a.passwordUpdated}
              </h1>
              <p className="mt-3 text-sm text-[#6a6a6a]">{a.passwordUpdatedSubtitle}</p>
              <LocalizedLink
                href="/login"
            ...
            <>
              ...
              <p className="mt-2 text-sm text-[#6a6a6a]">{a.chooseNewPasswordSubtitle}</p>
              ...
              <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
                ...
                >
                  {submitting ? a.updating : a.updatePassword}
                </button>
              </form>
            </>
          )}
