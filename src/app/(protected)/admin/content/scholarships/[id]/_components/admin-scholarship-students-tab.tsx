...
import type { AdminScholarshipStudentRow } from "../_lib/fetch-admin-scholarship-students-page";

const LIMIT_OPTIONS = [10, 20, 30, 50] as const;

export type AdminScholarshipStudentsTabProps = {
  title: string;
  emptyMessage: string;
  rows: AdminScholarshipStudentRow[];
  totalRows: number;
  page: number;
  limit: number;
...
export function AdminScholarshipStudentsTab({
  title,
  emptyMessage,
  rows,
  totalRows,
  ...
  limit,
  activityColumnLabel,
}: AdminScholarshipStudentsTabProps) {
  return (
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-[#fafaf8]">
              {["Student", "School", "Email", activityColumnLabel].map((heading) => (
                <th
                  key={heading}
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((row, index) => {
                ...
                return (
                  <tr key={row.studentId} className="transition-colors hover:bg-[#f0f7f2]">
