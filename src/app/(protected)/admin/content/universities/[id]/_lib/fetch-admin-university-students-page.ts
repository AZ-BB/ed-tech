...
export type AdminUniversityStudentRow = {
  studentId: string;
  name: string;
  email: string;
  school: string;
...
export async function fetchAdminUniversityStudentsPage(
  universityId: string,
  activityType: ActivityType,
  page: number,
  limit: number,
): Promise<{ rows: AdminUniversityStudentRow[]; totalRows: number }> {
  ...
  if (error) {
    console.error("[admin-university-students]", error);
  ...
  const rows: AdminUniversityStudentRow[] = slice.map((row) => {
