...
import type { AdminUniversityStudentRow } from "../_lib/fetch-admin-university-students-page";

const LIMIT_OPTIONS = [10, 20, 30, 50] as const;

export type AdminUniversityStudentsTabProps = {
  title: string;
  emptyMessage: string;
  rows: AdminUniversityStudentRow[];
  totalRows: number;
  page: number;
  limit: number;
...
export function AdminUniversityStudentsTab({
  title,
  emptyMessage,
  rows,
  totalRows,
  ...
  limit,
  activityColumnLabel,
}: AdminUniversityStudentsTabProps) {
  return (
