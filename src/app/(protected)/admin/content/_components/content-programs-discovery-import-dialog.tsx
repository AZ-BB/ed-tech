...
type ContentProgramsDiscoveryImportDialogProps = {
  open: boolean;
  onClose: () => void;
};
...
export function ContentProgramsDiscoveryImportDialog({
  open,
  onClose,
}: ContentProgramsDiscoveryImportDialogProps) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = await postFormImportWithSse<ImportSummary>(
        "/api/admin/programs-discovery/import",
        formData,
        {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      ...
      >
        ...
        <p className="mt-2 text-[13px] text-[#666]">
          Upload a multi-sheet Excel file. The first sheet is{" "}
          <strong>programs</strong>; other sheets map to JSONB sections. Programs
          are matched by <strong>program_id</strong> (slug).
        </p>
            ...
            <div className="space-y-3">
              <div className="rounded-[8px] border border-[#e0deda] bg-[#faf9f7] p-3 text-[13px] text-[#4a4a4a]">
                <p className="font-semibold text-[#1a1a1a]">
                  Imported {summary.programsUpserted} program
                  {summary.programsUpserted === 1 ? "" : "s"}
                </p>
