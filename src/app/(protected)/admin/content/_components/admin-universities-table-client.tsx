...
import type { AdminUniversityCountryOption } from "../_lib/fetch-admin-university-country-options";
import type { AdminUniversityTableRow } from "../_lib/fetch-admin-universities-page";
import {
  ADMIN_UNIVERSITIES_STATUS_FILTER_OPTIONS,
  type AdminUniversitiesStatusFilter,
} from "../_lib/parse-admin-universities-search-params";
...
function EditActionLink({ href }: { href: string }) {
  return (
    <Link
      href={href}
      title="View & edit"
      aria-label="View and edit university"
...
export type AdminUniversitiesTableClientProps = {
  rows: AdminUniversityTableRow[];
  totalRows: number;
  page: number;
  limit: number;
  q: string;
  ...
  countryOptions: AdminUniversityCountryOption[];
};

export function AdminUniversitiesTableClient({
  rows,
  totalRows,
  page,
  limit,
  ...
  status,
  countryOptions,
}: AdminUniversitiesTableClientProps) {
  const pathname = usePathname() ?? "";
  const router = useRouter();
  const filtersActive =
    q.trim().length > 0 || country.trim().length > 0 || status !== "";
    ...
    <div className="overflow-hidden rounded-[12px] border border-[#ece9e4] bg-white">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#ece9e4] px-5 py-4">
        <div className="flex min-w-0 items-center gap-2">
          <h2 className="text-[14px] font-bold text-[#1a1a1a]">Universities</h2>
        ...
        >
          ...
          <div className="relative w-full max-w-[220px]">
            ...
            <label htmlFor="admin-universities-search" className="sr-only">
              Search universities
            </label>
            <input
              id="admin-universities-search"
              key={`${q}-${country}-${status}`}
              type="search"
              name="q"
              defaultValue={q}
          ...
          <select
            ...
            defaultValue={status}
          >
            {ADMIN_UNIVERSITIES_STATUS_FILTER_OPTIONS.map((option) => (
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((row, index) => {
                ...
                const detailHref = getAdminUniversityDetailHref(row.id);
                ...
                return (
                  ...
                  >
                    ...
                    <td className={`${cellBorder} px-4 py-3 text-[13px] text-[#4a4a4a]`}>
                      {row.shortlistedCount.toLocaleString()} students
                    </td>
                    <td
