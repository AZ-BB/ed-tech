...
import { updateAdminInternship } from "@/actions/admin-internships";
import { AdminInternshipFormFields } from "@/app/(protected)/admin/content/_components/admin-internship-form-fields";
import { useRouter } from "next/navigation";
import { useEffect, useState } from "react";

import type { AdminInternshipDetailPayload } from "../_lib/fetch-admin-internship-detail";

type AdminEditInternshipDialogProps = {
  open: boolean;
  onClose: () => void;
  payload: AdminInternshipDetailPayload;
};

export function AdminEditInternshipDialog({
  open,
  onClose,
  payload,
}: AdminEditInternshipDialogProps) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await updateAdminInternship(formData);
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal
        aria-labelledby="edit-internship-title"
        ...
        >
          <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
            <AdminInternshipFormFields
              ...
              initial={payload.internship}
              internshipId={payload.internship.id}
              disabled={isSubmitting}
            />
          </div>
