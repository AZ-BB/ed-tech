...
export type AdminInternshipStudentRow = {
  studentId: string;
  name: string;
  email: string;
  school: string;
...
export async function fetchAdminInternshipStudentsPage(
  internshipId: string,
  page: number,
  limit: number,
): Promise<{ rows: AdminInternshipStudentRow[]; totalRows: number }> {
  ...
  if (error) {
    console.error("[admin-internship-students]", error);
  ...
  const rows: AdminInternshipStudentRow[] = slice.map((row) => {
