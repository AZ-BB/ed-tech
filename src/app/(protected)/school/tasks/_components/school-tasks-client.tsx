  ...
  return (
    <div className="space-y-4">
      <div className="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border-light)] bg-white">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--border-light)] px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-[12px] text-[var(--text-light)]">
              What needs to happen next, by when, and for which student (from My
              Applications tasks).
            </p>
          </div>
          <button
            type="button"
            className="inline-flex cursor-pointer items-center gap-2 rounded-lg border-[1.5px] border-[var(--green)] bg-[var(--green)] px-4 py-2 text-[12.5px] font-semibold text-white transition-colors hover:bg-[var(--green-dark)] disabled:cursor-not-allowed disabled:opacity-50"
        ...
        >
          ...
          <div className="relative min-w-[180px] flex-1 basis-[200px]">
            ...
            <label htmlFor="school-task-search" className="sr-only">
              Search tasks
            </label>
            <input
              id="school-task-search"
              type="search"
              name="q"
              defaultValue={q}
            ...
            <div className="space-y-[7px]">
              {rows.map((r) => {
                ...
                return (
                  <div
                    key={r.taskId}
                    className={`flex items-center gap-3 rounded-[10px] border border-[var(--border-light)] px-3.5 py-2.5 ${
                      r.completed ? "bg-[#faf9f4]" : "bg-white"
                    }`}
                  >
                    <button
                      type="button"
                      title={r.completed ? "Mark open" : "Mark complete"}
                      disabled={togglingId === r.taskId}
                      onClick={() => handleToggle(r)}
                      className={`mt-0.5 flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-[5px] border-[1.5px] border-[var(--border)] bg-white transition-colors hover:border-[var(--green-light)] disabled:cursor-wait disabled:opacity-50 ${
                        r.completed
