...
function rollupShortlist(
  ...
  for (const r of sorted) {
    ...
    if (p && !isUndecidedProgram(p) && acc.programs.length < 3) {
      acc.programs.push(p);
    }
  }
...
async function profilesToSchoolStudentTableRows(
  supabase: SchoolSupabase,
  profiles: StudentProfileListRow[],
): Promise<SchoolStudentTableRow[]> {
  ...
  if (appProfRes.error) {
    console.error(
      "[profilesToSchoolStudentTableRows] student_application_profile:",
...
/**
 * All students matching the same filters as the table, without pagination (chunked reads).
 */
export async function fetchAllSchoolStudentTableRows(
  supabase: SchoolSupabase,
  schoolId: string,
  filters: SchoolStudentsExportFilters,
): Promise<SchoolStudentTableRow[]> {
  ...
  const all: SchoolStudentTableRow[] = [];
  let offset = 0;
  ...
  for (;;) {
    ...
    const { data: profiles, error: listErr } = await listQuery;

    if (listErr) {
      console.error("[fetchAllSchoolStudentTableRows]", listErr);
      break;
    }
    if (!profiles?.length) break;

    const chunkRows = await profilesToSchoolStudentTableRows(
      supabase,
      profiles as StudentProfileListRow[],
    );
    all.push(...chunkRows);

    if (profiles.length < EXPORT_CHUNK) break;
    offset += EXPORT_CHUNK;
  }
...
export async function fetchSchoolStudentsPage(
  ...
  const rows = await profilesToSchoolStudentTableRows(
    supabase,
    profiles as StudentProfileListRow[],
  );
