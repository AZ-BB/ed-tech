...
export function EventSelectorBar({
  ...
  return (
    <div className="mb-5">
      <div className="mb-3.5 flex items-center gap-3 rounded-[18px] border-[1.5px] border-[var(--border-light)] bg-white px-5 py-4 transition focus-within:border-[var(--green-light)] focus-within:shadow-[0_0_0_4px_rgba(45,106,79,0.06)]">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-[18px] border border-[var(--border-light)] bg-white px-5 py-3">
        <FilterSelect
          label={labels.location}
          value={location}
          options={[{ value: "", label: labels.location }, ...locations.map((l) => ({ value: l, label: l }))]}
...
function FilterSelect({
  ...
  return (
    <select
      ...
      className="h-9 min-w-[120px] cursor-pointer appearance-none rounded-full border-[1.5px] border-[var(--border)] bg-white bg-[length:10px_6px] bg-[position:right_12px_center] bg-no-repeat py-2 pl-3.5 pr-8 text-[11.5px] leading-none text-[var(--text-mid)] outline-none transition hover:border-[var(--text-hint)] focus:border-[var(--green-light)]"
      style={{
        backgroundImage:
