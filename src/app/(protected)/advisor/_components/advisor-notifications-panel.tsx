...
import {
  markAllAdvisorNotificationsReadAction,
  markAdvisorNotificationReadAction,
} from "@/actions/advisor-notifications";
import {
  formatAdvisorNotificationTimestamp,
  type AdvisorNotificationItem,
...
type AdvisorNotificationsPanelProps = {
  open: boolean;
  onClose: () => void;
...
export function AdvisorNotificationsPanel({
  open,
  onClose,
  items,
  loading,
  ...
  onMarkAllLocalRead,
  onRefresh,
}: AdvisorNotificationsPanelProps) {
  ...
  const handleMarkAllRead = async () => {
    onMarkAllLocalRead();
    const res = await markAllAdvisorNotificationsReadAction();
    if (res.error) {
      await onRefresh();
    }
  };

  const handleItemClick = async (item: AdvisorNotificationItem) => {
    if (!item.read) {
      onMarkLocalRead(item.id);
      void markAdvisorNotificationReadAction(item.id);
    }
  ...
  return (
    <div
      id="advisor-notifications-panel"
      role="dialog"
      ...
      <div className={`max-h-[360px] overflow-y-auto px-2 py-2 ${SCROLL_THIN}`}>
        {loading && items.length === 0 ? (
          ...
          <ul className="flex flex-col gap-1">
            {items.map((item) => {
              const timeLabel = formatAdvisorNotificationTimestamp(item.createdAt);
              const content = (
