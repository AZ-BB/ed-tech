...
export default async function StandalonePaymentSuccessPage({
  searchParams,
}: PageProps) {
  ...
  if (sessionId) {
    const confirmed = await confirmStandalonePaymentFromSession(sessionId);
    if (!confirmed.ok) {
      console.error("[standalone payment success] confirm session", confirmed.error);
    }
  }
